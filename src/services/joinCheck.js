...
function extractUsername(targetLink) {
  if (!targetLink) {
    return null;
  }

  const trimmed = String(targetLink).trim();
  if (!trimmed) {
    return null;
  }
...
export async function joinCheck({ offer_id, tg_id, telegram }) {
  const offer = await db.one(`SELECT target_link FROM offers WHERE id=$1`, [offer_id]);
